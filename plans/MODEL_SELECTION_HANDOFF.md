...
验证环境是本项目固定的 `litellm==1.82.6`、`ollama_chat/qwen3.6:27b` 和 Ollama `http://127.0.0.1:11434`。结论如下：
...
- 因此首版两个本地 profile 的后端内部配置固定为：`local-qwen3.6-27b-direct -> reasoning_effort="none"`，`local-qwen3.6-27b-thinking -> reasoning_effort="low"`。只有本地 Qwen profile 传此参数；DeepSeek profile 不传。
